...
/** Lucid package root (where package.json + dist/ live). */
...
/**
 * Install Lucid Cursor hooks into a project so normal Agent chats are observed.
 * Merges with any existing `.cursor/hooks.json` entries.
 */
export async function installCursorHooks(options: {
  projectRoot: string
  /** Override Lucid package root (tests). */
  packageRoot?: string
  ...
  if (!(await exists(hookEntry))) {
    throw new Error(
      `Lucid hook entry missing at ${hookEntry}. Run \`npm run build\` in the Lucid package first.`,
    )
  }
...
/**
 * Lucid Cursor observe hook — keep prompting normally.
 * Generated by \`lucid attach cursor\`.
 */
...
`
  ...
  // Remember install metadata for debugging.
  const lucidDir = path.join(projectRoot, '.lucid')
  await mkdir(lucidDir, { recursive: true })
  await writeFile(
    path.join(lucidDir, 'cursor.json'),
    `${JSON.stringify(
      {
